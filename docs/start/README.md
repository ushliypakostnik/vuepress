...
* **Не использвать <code>scoped</code>** на компонентах-обертках. Тогда стили не будут «наслаиваться через хэши» и специфичность селектора по классу окажется выше чем специфичность «голого» элемента **ul** нормализации в базе.
