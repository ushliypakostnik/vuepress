...
С React все более прозаично - у меня просто есть папка <code>@/src/scss/components/</code> в которой мы складируем стили отдельных компонентов, которые, если это необходимо - задействуют медиатор, стилевую базу.
```
.
└─ src
   └─ sscs
...
Необходимо упомянуть, что некоторые разработчики применяют простую технологию [CSS Modules](https://github.com/css-modules/css-modules) для защиты стилей препроцессора в компонентах React.  
