...
$selector1__size: 200px;
$selector2__width: 200px;
$selector2__height: 400px;
...
.selector {
  ...
  &--2 {
    @include size($selector2__width, $selector2__height);
    /* ... включение других сущностей препроцессора
      и специфических правил уникальных для селектора */
  }
}
...
Vue позволяет защитить стили компонента через атбрибут **scoped**, захешировав слева от классов (классами можно перекрывать хеш), и изолировав их. Правда, иногда _все ломается_: исключение составляют компоненты являющиеся обертками над сторонними модулями, иногда со специфической проблемной кастомизацией и, на самом деле, вообще любые компоненты, использование которых c другими модулями затрудняет **scoped**.

Давайте на примере разберем простейший конфликт специфичности при использовании перенормализованной голой семантики и **scoped**. Вот в этом моем стартовом проекте, который мы еще будем ковырять в теме про темезацию [Vue c SCSS](https://github.com/ushliypakostnik/vue-scss-i18next/tree/master/src). 

У нас есть 3 компонента: Layuot - основная GUI-обертка, которая использует обертку Header со слотом в который передаются компонентны-переключатели. В данном случае, мы пишем быстрый прототип, стартовый шаблон, избегая излишней детализации. Для стилей оберток мы выставляем **scoped** в стилях компонента, а переключатели несут разный функционал, поэтому являсь очевидно разными компонентами, они, при этом, скорее всего, могут выглядеть одинаково и несут один и тот же класс для стилизации. Очень важно вот это понять: мы разделяем оформление-кастомизацию с помощью препроцессора и функциональность-логику интерфейса. Кроме того, так как это быстрый прототип, для разметки переключателя мы логично-семантично решаем использовать теги списка.
...
.switch {
  ...
  padding: ($gutter / 4) ($gutter / 2) !important; // перекрываем base в хешах
...
![Картинка](../../assets/image--1.jpg)

Прежде всего, мы обнаруживаем что хеш стилей от Header - встает правее чем собственный класс. Но главная суть проблемы и конфликта не в этом. Дело в том, что, прежде всего, мы вероятно слишком увлеклись нормализацией в файле **_base.scss** стилевой базы:
```scss
...
Таким образом, даже если мы отменим хещирование стилей для Header, все равно специфичность голого тага-элемента с атрибутом **ul[data-v-xxxxxxxx]** из левого хеша от Layout оказывается выше чем у простого селектора по классу. Также вот можно наблюдать «наслоение правил от стилевой базы» которое дает компонентность.

В этой ситуации мы можем:
* Самый адекватный вариант: убрать обнуление внутренних отступов из нормализации, если мы собираемся использовать голую семантику списков - слишком навязчиво будет что-то делать с ее оформлением в стилевой базе;  
* Самый раздолбайский вариант: использовать **!important** как показано выше и не париться - жизнь и так слишком коротка чтобы переживать из-за незначительных мелочей!)));
* Понятно: не использовать семантику списка - сверстать на классах;
* Не использвать **scoped** на компонентах-обертках. Тогда стили не будут «наслаиваться через хеши» и специфичность селектора по классу окажется выше чем специфичность «голого» элемента **ul** нормализации в базе.
...
Мы легко можем использовать javascript для того чтобы определить объект с глобальными константами на которые в дальнейшем и будем опираться в единообразном и управляемом оформлении компонент. Тоесть, _сформировать гайдлайн_, и, например, стандартизировать в нем цвета и брекпоинты.
...
* Наглядные **однофайловые компоненты**: с изолируемыми только для компонента **scoped**-стилями на любом препроцессоре прямо в коде однофайлового компонента;
* Очень важно для целей библиотеки - проработанная развитая **система менеджента контента** - [Слоты](https://ru.vuejs.org/v2/guide/components-slots.html);
* При необходимости, тоже очень удобно - проработаные готовые **переходы и анимации** для компонентов - на уровне JSX;
* Наличие развитой **экосистемы для документирования**: смотрите [VuePress](https://vuepress-lrouuhpdsl.now.sh/ru/) на котором написано это руководство.

Еще некоторые особенности и преимущества прогрессивного фреймворка Vue будут расмотрены ближе к концу второй части этого пособия.
...
* Тоталитарно относятся к вашему стилю кода, стандартам и подходам, агрессивно навязывают свои правила, изначально очевидно излишни и громозки;
