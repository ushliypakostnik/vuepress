...
С React все более прозаично - у нас просто есть папка <code>@/src/scss/components/</code> в которой мы складируем стили отдельных компонентов, которые, если это необходимо - просто используют стилевую базу.
```
.
└─ src
   └─ sscs
