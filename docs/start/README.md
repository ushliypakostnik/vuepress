...
Прежде всего, мы обнаруживаем что хэш стилей от Header - встает правее чем собственный класс компонента переключателя. Но главная суть проблемы и конфликта не в этом. Дело в том, что мы, вероятно, слишком увлеклись нормализацией в файле <code>@/src/scss/_base.scss</code> стилевой базы:
```scss
