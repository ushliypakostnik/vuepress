...
Спросите любого нормального дизайнера: «Что в стиле и оформлении веб-интерфейса (тоесть в UI, а не в UI/UX) самое главное?». Думаю, многие ответят: «Типографика». Многим заказчикам и пользователям нравятся «рюшечки и котики», или, что очень важно - **контент который обеспечивает ваша верстка должен быть уверено доступен с клавиатуры и без мыши, и разметка должна быть дружественна для слабовидящих людей**. Но все же, типографика и текстовый контент категорически важны, текстовые поля должны быть с перманентно одинаковым оформлением по всему интерфейсу, по крайней мере.
...
Или, даже так, при необходимости, хотя это не и «не по БЭМ» - дальше об это подробнее:
...
Давайте разберемся как определять отношения между более высокоурневыми астракциями и низкоуровневыми функциональными модулями не нарушая идеальной компонентности. Ведь делать это почти всегда все равно необходимо.

Представим нам что-то нужно сделать с переиспользуемым компонентом **.сomponent** во всем интерфейсе только на iOS. У крупной обертки **.layout** будет через javascript выставляться модификатор **.layout--iOS**:
```html
...
Если же речь идет о разметке конкретного отдельного вида [а не «уровне всего интерфейса»], правильно будет использовать стили этого конкретного вида:
```html
<section id="view">
...
На самом деле ведь очень просто и четко - если цель связывания - воздействие на компонент - отдаем стили ему. Если цель - рядовое использование компонента в неком обособленном контексте - пишем в этот выделенный код. Если вы все поймете и сделаете правильно - идеальная компонентность не будет нарушаться при таких связываниях.

Хорошо будет если даже порядок объявлений в селекторах будет как-то минмально упорядочен. На этот счет есть несколько правил, для которых невозможно насторить линтер, но которым я стараюсь следовать:
