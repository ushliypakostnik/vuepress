...
```
**3.** Иногда можно вкладывать селекторы в более крупные обёртки, классы-абстракции, позволяющие выделять и защищать стили в крупных частях проекта: это различные страницы-шаблоны, темы, версии. Такой подход позволяет добиваться большей компактности и переиспользуемости кода, по необходимости легко влиять на поведение нужных более мелких компонентов: конструкций сетки, виджетов, утилит в любом месте проекта.
```scss
// Смотря где удобнее это сделать - в самом селекторе или в обертке:
