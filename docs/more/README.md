...
Для разметки видов кажется вполне оправданным использовать идентефикатор - по спецификации - то что точно присутствует на странице в единственном экземпляре.
...
Давайте запилим большой важный классовый компонент <code>Layout</code> для React c роутером, который будет все это использовать, уметь отличать iOS и плюс еще реагировать на скролл - крайне распрастранненный кейс, например, нужно поменять оформление, добавить класс на <code>Header</code> только на десктопах:
