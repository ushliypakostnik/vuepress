...
Вы, возможно, заметили что в коде [ради лаконичности примеров] не используется конструктор и локальный  для классовых компонентов. Состояние стора синхронизируется с <code>this.props</code> класса. Работать будет, но вы увидите вот такой warning в консоли:
