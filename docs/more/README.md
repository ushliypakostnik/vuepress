...
В, этой, концептуально - ключевой для всего трактата главе, хочется прежде всего поговорить о самых рамочных, важных абстрактных идеях современного и, возможно, даже будущего экранного дизайна. 
...
Давайте пофиксим _раз и навсегда_. Для начала, чтобы вы все знали и понимали об этом, представим, что мы верстаем обычный лэндинг, «по классике», с настолько минимальной функциональсностью, что использовать фреймворк очевидно лишнее и ни для чего вообще не нужно. Как можно защитить на нем дизайн, зависищий от скриптов? Выполнять все манипуляции со стилями каждый раз после того как размер окна изменился. Используем [IIFE](https://developer.mozilla.org/ru/docs/%D0%A1%D0%BB%D0%BE%D0%B2%D0%B0%D1%80%D1%8C/IIFE) в актуальном es6-синтаксисе: 
...
Теперь стор знает о типоразмере которому в данный момент соответсвует вьюпорт. Вы можете добавлять любые поля в хранилище и специфическую логику в модуль.

Когда мы работаем с реактивным фреймворком речь идет о лаконично, логично и четко организованной поставленной системе. И чтобы она дейтсвительно работало «как часы» - не стоит ничего слишком усложнять! Я вижу это так, что у нас есть только: 
