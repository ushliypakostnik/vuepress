...
Но **никогда** не делайте так в обработчике на скролл - зажмет нахрен!) Ширина скролла это в принципе такая вещь, которую стоит учитывать в хорошем адаптивном веб-дизайне, но приемлимо - вычислить и записать один раз. Для классового компонента React (но если может понадобиться больше одного раза - правильно сделать это через специализированный модуль с конектом к стору - такой будет показан дальше в теме про ресайз): 
