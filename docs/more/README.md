...
Обратите внимание - десктопные браузеры имеют разную ширину основного скроллбара - от нулевой прозрачной, до, возможно, даже некой специфической, кастомизированной через CSS для webkit. Поэтому если вашему javascript потребуется сравнить значение <code>document.documentElement.clientWidth</code> с брекпоинтом - вполне вероятно он будет ошибаться на ненулевую ширину в части просмотрщиков. В таких случаях можно использовать уточняющую логику:

```javascript
if (document.documentElement.clientWidth < ScreenHelper.DESKTOP - ScreenHelper.getScrollbarWidth()) {
  // Логика для гаджетов
}
```

Но **никогда** не делайте так в обработчике на скролл - зажмет нахрен!) Ширина скролла это в принципе такая вещь которую стоит учитывать в хорошем адаптивном веб-дизайне, но приемлимо - учесть и записать один раз. Если это классовый компонент React, то: 
...
Давайте запилим большой важный классовый компонент <code>Layout</code> для React c роутером, который будет все это использовать, уметь отличать iOS и плюс еще уметь реагировать на скролл - крайне распрастранненный кейс, например, нужно поменять оформление, добавить класс на <code>Header</code> только на десктопах:
