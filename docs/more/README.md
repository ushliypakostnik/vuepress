...
До сих пор, несмотря на очевидный прогресс спецификации и переход в повседневную практику верстальщиков таких мощных средств как, например, нативные раскладки пространства, относительные единицы или <code>calc()</code>, не все «хотелки» дизайнеров и заказчиков удается уверенно обслужить через CSS. То, что не тянут стили, закономерно ложиться на стрипты. В реальности по-прежнему часто необходимо манипулировать оформлением на DOM-элементах с помощью javasript. После сборки DOM, первого рендера бывает нужно еще немного «помочь стилям», осуществив некие, основанные на параметрах среды, вьюпорта, подсчеты и выставить правила с верными значениями прямо в инлайн-стили некоторых элементов. А что если пользователь после этого изменит размер вьюпорта, тоесть произойдет **resize**? Легко может _все сломаться_ и надо будет фиксить. 
...
На практике, иногда вам еще может потребоваться нулевой или ненулевой таймаут в событии <code>load</code> или даже, в самых тяжелых случаях, <code>resize</code>, для того чтобы скрипты не ошибались в подсчетах:
```javascript
setTimeout(() => {
  redraw();
}, 0); // если нулевой не помогает - ищи минимальное значение
```
...
Проблема в том, что если мы будем использовать единицы относительно высоты <code>vh</code> - браузер будет ошибаться, так как «ничего не знает» о нативных панелях устройства которые накладываются на вьюпорт сверху. Но с помощью javascript мы можем после первого рендера быстро посмотреть реальный размер полезного окна и выставить всему документу скорректированное значение через кастомное свойство:   
...
Теперь добавим правильное поведение в компоненте самого контрола - нажатие на Enter должно активировать правильное поведение, такое же как и по клику:
...
class Control extends PureComponent {
  ...
  // Фикс для добавления правильного поведения по нажатию Enter
  onKeyPress = (e) => {
